...
`src/vulkan_ngx_loader.*` implements the safe loading and lifecycle part of the
NGX layer. It dynamically loads `nvngx_dlssg.dll` and verifies the complete
Vulkan export surface found in the shipped runtime: base and extended init,
feature creation (including `CreateFeature1`), feature requirements, scratch
buffer sizing, evaluation, release, and both shutdown variants. It also tracks
device creation and swapchain changes and rejects stale or cross-device
lifecycle events. It deliberately does not cast or call proprietary NGX
function signatures; the host must provide the matching SDK headers/adapter
before invoking those addresses.
